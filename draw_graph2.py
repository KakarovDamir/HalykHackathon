...
def return_graph(start_date='2023-01-26', end_date='2023-11-06', product_name='Iphone 14 Case', category='income'):
    x_os = []
    y_os = []
    ...
    fig.write_html(f"plotly_figure2.html")
...
def check():
    while True:
        ...
        csv_files = [f.name for f in os.scandir(f_path) if f.name.endswith('.csv') and f.is_file() and f.name[0:6] == 'graph2']
        if len(csv_files) > 0:
            prev = csv_files[0]
